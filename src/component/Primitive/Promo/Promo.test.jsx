...
describe('Component: Promo', () => {
  test('should return errors if required props missing', () => {
    ...
    const actual = validatePropTypes(Promo.propTypes, {})
    const expected = {
      children:
  ...
  test('shouldn’t error if valid default props passed', () => {
    // eslint-disable-next-line react/forbid-foreign-prop-types
    const actual = validatePropTypes(Promo.propTypes, requiredProps())
    const expected = undefined
    expect(actual).toEqual(expected)
  })

  test('should output the expected markup with default props', () => {
    const { getByText } = render(<Promo {...requiredProps()} />)
    expect(getByText('Default content')).toBeTruthy()
  })

  // test('should output additional className when `foo` prop passed', () => {
  //   const { } = render(<Promo {...requiredProps()} foo />)
  //   expect().toEqual()
  // })
})
